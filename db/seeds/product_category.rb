...
parent = ProductCategory.create(name: 'メンズ')
...
parent = ProductCategory.create(name: 'ベビー・キッズ')
...
parent = ProductCategory.create(name: 'インテリア・住まい・小物')
...
parent = ProductCategory.create(name: '本・音楽・ゲーム')
...
parent = ProductCategory.create(name: 'おもちゃ・ホビー・グッズ')
...
parent = ProductCategory.create(name: 'コスメ・香水・美容')
...
parent = ProductCategory.create(name: '家電・スマホ・カメラ')
appliance_child_array.each.each_with_index do |child, i|
  child = parent.children.create(name: child)
  appliance_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
parent = ProductCategory.create(name: 'ハンドメイド')
...
parent = ProductCategory.create(name: 'チケット')
...
#孫カテゴリー配列
bike_grandchild_array = [['すべて','国内自動車本体','外国自動車本体'],['すべて','タイヤ/ホイールセット','タイヤ','ホイール','その他'],['すべて','サスペンション','ブレーキ','外装、エアロパーツ','ライト','内装品、シート','ステアリング','マフラー・排気系','エンジン、過給器、冷却装置','クラッチ、ミッション、駆動系','電装品','補強パーツ','汎用パーツ','外国自動車用パーツ','その他'],['すべて','車内アクセサリー','カーナビ','カーオーディオ','車外アクセサリー','メンテナンス用品','チャイルドシート','ドライブレコーダー','レーダー探知機','カタログ/マニュアル','セキュリティ','ETC','その他'],['すべて'],['すべて','タイヤ','マフラー','エンジン、冷却装置','カウル、フェンダー、外装','サスペンション','ホイール','シート','ブレーキ','タンク','ライト、ウィンカー','チェーン、スプロケット、駆動系','メーター','電装系','ミラー','外国オートバイ用パーツ','その他'],['すべて','ヘルメット/シールド','バイクウエア/装備','アクセサリー','メンテナンス','カタログ/マニュアル','その他']]

parent = ProductCategory.create(name: '自動車・オートバイ')
bike_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
  bike_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
